...
function _ThirdwebConnect(taskId, wallet, chainId, cb) {
 var id = UTF8ToString(taskId);
 var idSize = lengthBytesUTF8(id) + 1;
 var idPtr = _malloc(idSize);
 stringToUTF8(id, idPtr, idSize);
 window.bridge.connect(UTF8ToString(wallet), chainId).then(address => {
  if (address) {
...
var dynCall_viifififfffii = Module["dynCall_viifififfffii"] = createExportWrapper("dynCall_viifififfffii");
